...
public class CarController {
    ...
    public ResponseEntity<Object> getCarsByClientLocation(@PathVariable int clientId) {
        List<Car> cars = carService.getCarByClientLocation(clientId);
        if (cars.isEmpty()) {
            return AppResponse.error()
                    .withMessage("No cars available for the client's location.")
                    .withCode(HttpStatus.NOT_FOUND)
                    .build();
        }
        return AppResponse.success()
                .withMessage("Cars retrieved successfully.")
                .withData(cars)
